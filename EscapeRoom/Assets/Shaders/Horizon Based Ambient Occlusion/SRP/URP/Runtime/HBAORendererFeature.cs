...
namespace HorizonBasedAmbientOcclusion.Universal
{
    public class HBAORendererFeature : ScriptableRendererFeature
    {
        private class HBAORenderPass : ScriptableRenderPass
        {
            ...
            {
                BlitFullscreenTriangle(cmd, source, ShaderProperties.inputTex, material, Pass.Copy);
            }

            private void AO(CommandBuffer cmd)
            {
                BlitFullscreenTriangleWithClear(cmd, hbao.mode.value == HBAO.Mode.LitAO ? source : ShaderProperties.inputTex, ShaderProperties.hbaoTex, material, new Color(0, 0, 0, 1), Pass.AO);
            }
            ...
            private void DeinterleavedAO(CommandBuffer cmd)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    BlitFullscreenTriangle(cmd, BuiltinRenderTextureType.CameraTarget, m_RtsDepth, material, Pass.Deinterleave_Depth); // outputs 4 render textures
                    BlitFullscreenTriangle(cmd, BuiltinRenderTextureType.CameraTarget, m_RtsNormals, material, Pass.Deinterleave_Normals); // outputs 4 render textures
                }
                ...
                for (int i = 0; i < 4 * 4; i++)
                {
                    ...
                    BlitFullscreenTriangleWithClear(cmd, hbao.mode.value == HBAO.Mode.LitAO ? source : ShaderProperties.inputTex, ShaderProperties.aoSliceTex[i], material, new Color(0, 0, 0, 1), Pass.AO_Deinterleaved); // ao
            ...
            private void Blur(CommandBuffer cmd)
            {
                if (hbao.blurType.value != HBAO.BlurType.None)
                {
                    ...
                    BlitFullscreenTriangle(cmd, ShaderProperties.hbaoTex, ShaderProperties.tempTex, material, Pass.Blur);
                    cmd.SetGlobalVector(ShaderProperties.blurDeltaUV, new Vector2(0, 1f / height));
                    BlitFullscreenTriangle(cmd, ShaderProperties.tempTex, ShaderProperties.hbaoTex, material, Pass.Blur);
                    cmd.ReleaseTemporaryRT(ShaderProperties.tempTex);
                }
            }
            ...
            private void TemporalFilter(CommandBuffer cmd, CameraHistoryBuffers buffers)
            {
                if (hbao.temporalFilterEnabled.value && !renderingInSceneView && buffers != null)
                {
                    ...
                    if (hbao.colorBleedingEnabled.value)
                    {
                        ...
                        BlitFullscreenTriangle(cmd, previousFrameAORT, rts, viewportRect, material, Pass.Temporal_Filter);
                        //cmd.SetViewProjectionMatrices(cameraData.camera.worldToCameraMatrix, cameraData.camera.projectionMatrix);
                    ...
                    else
                    {
                        ...
                        BlitFullscreenTriangle(cmd, previousFrameRT, currentFrameRT, viewportRect, material, Pass.Temporal_Filter);
