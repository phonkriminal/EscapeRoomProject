...
public class Settings_Menu : MonoBehaviour
{
	...
	void Start ()
	{
		...
		if (PlayerPrefs.GetInt("Frame Rate") == 0)
			limitFrameRate.isOn = false;
		else
			limitFrameRate.isOn = true;
	...
	public void Set_Resolution_Quality ()
	{
		if (resolutionQuality.value == 3)
		{
            ...
            Debug.Log($"Width: {(int)(PlayerPrefs.GetInt("OriginalX"))}  Height: {(int)(PlayerPrefs.GetInt("OriginalY"))}");
		}
    ...
    {
		StartCoroutine(FrameRate_Limiter());
    }
	IEnumerator FrameRate_Limiter()
	{
		yield return new WaitForEndOfFrame();
		if (limitFrameRate.isOn)
			PlayerPrefs.SetInt("Frame Rate", 1);  // 1 = true;
		else
			PlayerPrefs.SetInt("Frame Rate", 0);// 0 = false;


		if (PlayerPrefs.GetInt("Frame Rate") == 1)
