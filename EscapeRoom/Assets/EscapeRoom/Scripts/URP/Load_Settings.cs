...
public class Load_Settings : MonoBehaviour
{
	...
	void Start ()
	{
		...
		if (PlayerPrefs.GetInt("The First Run") != 1) // 1 = true; others = 0
		{
			...
			PlayerPrefs.SetInt("OriginalX", Screen.currentResolution.width);

			PlayerPrefs.SetInt("OriginalY", Screen.currentResolution.height);
			PlayerPrefs.SetInt("Frame Rate", 0);
	...
	// Call this from settings menu script tp update display fps settings durring menu
